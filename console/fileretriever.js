        ...
        if (status == 200) {
            ...
            ajax.get(indirectUrl, me.headers, function(data, status) {
                if (status == 200) {
                    ...
                    console.log('错误：加载错误，url： "' + url + '"', status, data);
                    err = new Error('错误：加载URL失败，url：  "' + url + '"');
                    callbackOnce(err, null);
                }
            });
        }
...
FileRetriever.prototype.prompt = function (params) {
    ...
    cancel.type = 'button';
    cancel.value = params.titleCancel || '取消';

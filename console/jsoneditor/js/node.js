    ...
    if (domValue) {
        ...
        if (t == 'array' || t == 'object') {
            ...
            if (this.editor.mode.edit) {
                domValue.title = '使用 Ctrl+Click 或者 Ctrl+Enter 在新窗口打开URL';
            }
        }
        else {
            domValue.title = '';
...
Node.prototype.getDom = function() {
    ...
    if (this.editor.mode.edit) {
        ...
        if (this.parent) {
            ...
            domDrag.title = '上下拖动移动字段位置(Alt+Shift+Arrows)';
...
Node.TYPE_TITLES = {
    'auto': '字段类型 "auto". ' +
        '该字段类型会自动定义值，可以是' +
        'string, number, boolean, 或者 null',
    'object': '字段类型 "object". ' +
        ' object 包含 key/value 对的无序集合',
    'array': '字段类型 "array". ' +
        ' array 包含有序的集合',
    'string': '字段类型 "string". ' +
        '该字段类型不会自动定义值, ' +
...
Node.prototype.showContextMenu = function (anchor, onClose) {
    ...
    if (this.parent && this.parent._hasChilds()) {
        ...
        if (node == childs[childs.length - 1]) {
            items.push({
                'text': '添加',
                'title': '在该字段后使用 \'auto\' 类型添加新的字段(Ctrl+Shift+Ins)',
                'submenuTitle': '选择添加的字段类型',
                'className': 'append',
                'click': function () {
                    node._onAppend('', '', 'auto');
                },
        ...
        items.push({
            'text': '插入',
            'title': '在该字段前插入一个\'auto\'类型的新字段(Ctrl+Ins)',
            'submenuTitle': '选择要插入的字段类型',
            'className': 'insert',
            'click': function () {
